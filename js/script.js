...
// Prevent Lucide from double-processing elements by removing data-lucide after replacement
if (window.lucide && !window.lucide.createIcons.wrapped) {
    ...
    lucide.createIcons.wrapped = true;
}

const initApp = () => {
    // Inject Demo Modal if not present on the page
    if (!document.getElementById('demoModal')) {
        const modalDiv = document.createElement('div');
        modalDiv.id = 'demoModal';
        modalDiv.className = 'fixed inset-0 bg-black/60 hidden items-center justify-center z-[100] transition-opacity duration-300 opacity-0';
        modalDiv.innerHTML = `
            <div class="modal-content bg-white p-10 rounded-[2rem] max-w-lg w-full m-4 shadow-2xl relative transition-transform duration-300 transform scale-95">
                <button onclick="closeModal()" class="absolute top-6 right-6 text-gray-400 hover:text-gray-800">
                    <i data-lucide="x" class="w-6 h-6"></i>
                </button>
                <h3 class="text-3xl font-bold text-primary-blue mb-4">Book Your Free Trial</h3>
                <form data-validate class="space-y-4">
                    <input type="text" required placeholder="Name"
                        class="w-full px-5 py-3 border rounded-xl outline-none focus:ring-2 ring-primary-blue/20">
                    <input type="email" required placeholder="Email"
                        class="w-full px-5 py-3 border rounded-xl outline-none focus:ring-2 ring-primary-blue/20">
                    <button type="submit" class="w-full btn-skillora py-4 rounded-xl">Submit</button>
                    <div class="success-msg hidden text-center font-bold text-green-600">Success! We'll call you.</div>
                </form>
            </div>
        `;
...
// --- Global Logout Logic ---
const setupLogout = () => {
    const logoutBtns = document.querySelectorAll('#logoutBtn, .sidebar-logout');
...
// Robust readyState-aware application loader
